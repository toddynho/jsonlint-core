...
// ---- 4. Pathological inputs (no crash, bounded time, capped diagnostics) ----
{
  ...
  check("100k closers no crash", t(() => validate("]".repeat(100000))) < 2000);
  const commas = ",".repeat(500000);
  check("500k commas bounded", t(() => validate(commas)) < 3000);
  const manyDup = "{" + Array.from({length: 5000}, (_, i) => `"k${i % 10}":1`).join(",") + "}";
  check("5k members w/ dup detection bounded", t(() => validate(manyDup)) < 3000);
  const longStr = '"' + "a".repeat(5_000_000) + '"';
  check("5MB single string fast", t(() => validate(longStr)) < 1000);
  const nulls = "\u0000".repeat(200000);
  check("200k NULs no crash", t(() => validate(nulls)) < 2000);
